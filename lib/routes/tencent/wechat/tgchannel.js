...
module.exports = async (ctx) => {
    ...
    const mpName = ctx.params.mpName || null;
    ...
    const out = await Promise.all(
        list
            .map(async (index, item) => {
                ...
                const link = item.find('.tgme_widget_message_text > a:nth-child(5)').attr('href') || item.find('.tgme_widget_message_text > a:nth-child(2)').attr('href'); // 为了不破坏现有订阅，没法替换它为 https
                ...
                if (link !== undefined) {
                    ...
                    if (value) {
                        ...
                        try {
                            const response = await got.get(link.replace(RegExp('^http://'), 'https://')); // 替换为 https 以避免跳转
