    ...
    formatArticle: async (ctx, itemDesc, status) => {
        ...
        if (pageInfo && pageInfo.type === 'article' && pageInfo.page_url) {
            ...
            if (article && article.title && article.content) {
                ...
                html += `<h2 style="font-size: 1.5rem;line-height: 1.25;color: #333;">${title}</h2>`; // 加入标题
                ...
                let articleMeta = '<div style="line-height: 1.66; color: #999;margin-bottom: 0.75rem;font-size: 0.75rem;">';
                if (isArticleNonFree) {
                ...
                if (summary) {
                    html += `<div style="color: #999;line-height: 1.5rem;padding: 0.0625rem 0 0.875rem;">${summary}</div>`; // 摘要
                }
